...
DocumentService.ready().then(() => {
  ...
  const highlightCurrentWeekday = () => {
    ...
    const currentWeekday = (today2.getDay() - calendarOptions.firstDay + 7) % 7;
    const weekdayHeaders = container.querySelectorAll(".ec-header .ec-days .ec-day");
    weekdayHeaders.forEach((header, index4) => {
      header.classList.toggle("active", index4 === currentWeekday);
    });
  };
